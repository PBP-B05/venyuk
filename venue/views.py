...
def book_venue(request, venue_id):
    ...
    try:
        ...
        # PERBAIKAN: Validasi tanggal tidak boleh hari ini atau masa lalu (harus H+1)
        today = date.today()
        if booking_date <= today:
            return JsonResponse({
                'success': False,
                'message': 'Tidak bisa booking untuk hari ini atau tanggal yang sudah lewat. Minimal booking untuk besok.'
            }, status=400)
